...
window.displayReviewsInModal = function(productId) {
    ...
    container.innerHTML = `
        <div class="reviews-summary" style="display:flex; gap:30px; padding:15px 0; border-bottom:1px solid #eee; margin-bottom:20px; flex-wrap:wrap;">
            <div style="text-align:center;">
                <div style="font-size:32px; font-weight:900; color:#111;">${avgRating.toFixed(1)}</div>
                <div style="display:flex; gap:3px; direction:ltr; font-size:16px;">
                    ${Array(5).fill().map((_, i) => 
                        `<i class="fas fa-star" style="color: ${i < Math.round(avgRating) ? '#FFD700' : '#ddd'};"></i>`
                    ).join('')}
                </div>
                <div style="font-size:13px; color:#888;">${productReviews.length} تقييم</div>
            </div>
                ...
                ${[5,4,3,2,1].map(star => {
                    ...
                    return `
                        <div style="display:flex; align-items:center; gap:8px; font-size:12px; margin:3px 0;">
                            <span style="min-width:30px;">${star} <i class="fas fa-star" style="font-size:10px;"></i></span>
                            <div style="flex:1; height:6px; background:#eee; border-radius:10px; overflow:hidden;">
                                <div style="width:${percent}%; height:100%; background:#FFD700; border-radius:10px;"></div>
                            </div>
                            <span style="min-width:30px; color:#888;">${ratingCounts[star]}</span>
                        </div>
                    `;
                }).join('')}
            </div>
            ...
            ${productReviews.map(review => `
                <div style="padding:12px 0; border-bottom:1px solid #f5f5f5;">
                    <div style="display:flex; align-items:center; gap:10px; margin-bottom:4px;">
                        <div style="width:32px; height:32px; border-radius:50%; background:linear-gradient(135deg,#2c3e50,#1a1a2e); display:flex; align-items:center; justify-content:center; color:#D4AF37; font-weight:800; font-size:14px;">
                            ${(review.userName?.charAt(0) || '?').toUpperCase()}
                        </div>
                        <div>
                            <div style="font-weight:700; font-size:14px;">${escapeHtml(review.userName || 'مستخدم')}</div>
                            <div style="display:flex; gap:2px; direction:ltr; font-size:13px;">
                                ${Array(5).fill().map((_, i) => 
                    ...
                    <div style="font-size:14px; color:#555; line-height:1.7;">${escapeHtml(review.text || '')}</div>
                    <div style="font-size:11px; color:#999; margin-top:4px;">${review.createdAt?.toDate ? formatDate(review.createdAt.toDate()) : ''}</div>
                </div>
            `).join('')}
        </div>
    `;
...
window.submitReview = function() {
    ...
    if (!window.selectedRating || window.selectedRating === 0) {
        showToast('⚠️ يرجى اختيار تقييم بالنجوم أولاً', 'warning');
        return;
    }
    
    const reviewText = document.getElementById('reviewText').value.trim();
    if (!reviewText) {
        showToast('✍️ يرجى كتابة رأيك في المنتج قبل إرسال التقييم', 'warning');
...
window.submitRating = async function() {
    ...
    try {
        ...
        showToast('✅ تم إرسال تقييمك بنجاح! سيظهر بعد المراجعة.', 'success');
        ...
        showToast('❌ حدث خطأ أثناء إرسال التقييم، حاول مرة أخرى.', 'error');
    } finally {
        const btn = document.querySelector('.rating-submit-btn');
        btn.disabled = false;
...
const infoContent = {
    shipping: `
        ...
        <p><strong>تكلفة الشحن:</strong> مجاني للطلبات فوق 1000 جنيه.</p>
        
        <h4 style="margin-top:25px;">🔄 سياسة الإرجاع والاستبدال</h4>
    ...
    size: `
        <h4>📏 دليل المقاسات</h4>
        <div class="size-tabs">
            <button class="size-subtab active" data-subtab="rings">💍 الخواتم</button>
            <button class="size-subtab" data-subtab="bracelets">📿 الغوايش</button>
            <button class="size-subtab" data-subtab="necklaces">✨ السلاسل</button>
        </div>
        
        <div id="ringsTab" class="size-subcontent active">
            <h5>قياس الخواتم</h5>
            <p><strong>الطريقة الأولى (القطر الداخلي):</strong> ضعي خاتم مناسب على يدك، ثم قيسي المسافة من الداخل.</p>
            <table class="size-guide-table">
                <thead><tr><th>القطر (مم)</th><th>المقاس</th></tr></thead>
                <tbody>
            ...
            <p><strong>الطريقة الثانية (محيط الخيط):</strong> لفي خيطاً حول اصبعك (بدون شد)، قيسي الطول.</p>
            <table class="size-guide-table">
        ...
        <div id="braceletsTab" class="size-subcontent">
        ...
        <div id="necklacesTab" class="size-subcontent">
            <h5>قياس السلاسل</h5>
            ...
            <table class="size-guide-table">
                ...
                <tbody>
                    ...
                    <tr><td>50</td><td>عظمة الترقوة</td></tr>
                    <tr><td>55</td><td>أسفل الترقوة</td></tr>
        ...
        <div style="margin-top:20px; padding:15px; background:#f9f8f6; border-radius:12px; border-right:3px solid #D4AF37;">
    ...
    faq: `
        <div class="faq-item">
            ...
            <div class="faq-answer">نعم، جميع منتجاتنا مصنوعة من معادن ثمينة (ستانلس ستيل بيور) ومضمونة 100% أصلية.</div>
        </div>
        <div class="faq-item">
            <div class="faq-question" onclick="this.parentElement.classList.toggle('open')">
                ❓ كيف أتابع طلبي بعد الشراء؟ <i class="fas fa-chevron-down"></i>
            </div>
            <div class="faq-answer">سنرسل لك تأكيد الطلب عبر واتساب يمكنك من خلاله تتبع حالة الطلب.</div>
        </div>
        <div class="faq-item">
            <div class="faq-question" onclick="this.parentElement.classList.toggle('open')">
                ❓ ما هي طرق الدفع المتاحة؟ <i class="fas fa-chevron-down"></i>
            </div>
            <div class="faq-answer">الدفع عند الاستلام (كاش) لجميع المحافظات، أو تحويل محفظة إلكترونية حسب الاتفاق.</div>
        </div>
        <div class="faq-item">
            <div class="faq-question" onclick="this.parentElement.classList.toggle('open')">
                ❓ مدة التوصيل؟ <i class="fas fa-chevron-down"></i>
            </div>
            <div class="faq-answer">تتراوح بين 3-7 أيام حسب محافظتك.</div>
        </div>
        <div class="faq-item">
            <div class="faq-question" onclick="this.parentElement.classList.toggle('open')">
                ❓ هل يمكن تغيير المقاس بعد الشراء؟ <i class="fas fa-chevron-down"></i>
            </div>
            <div class="faq-answer">نعم، يمكنك استبدال المقاس خلال 14 يومًا من الاستلام بشرط أن يكون المنتج غير مستخدم.</div>
        </div>
    `
};
...
function renderInfoTab(tabId) {
    ...
    if (tabId === 'size') {
        document.querySelectorAll('.size-subtab').forEach(tab => {
            tab.addEventListener('click', function() {
                document.querySelectorAll('.size-subtab').forEach(t => t.classList.remove('active'));
                this.classList.add('active');
                document.querySelectorAll('.size-subcontent').forEach(c => c.classList.remove('active'));
                const target = document.getElementById(this.dataset.subtab + 'Tab');
